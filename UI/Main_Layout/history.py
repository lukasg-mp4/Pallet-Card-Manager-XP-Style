...
class HistoryWindow(tk.Toplevel):
    def __init__(self, parent, app):
        ...
        # --- FIX: Apply XP Styling to Treeview ---
        style = ttk.Style()
        style.theme_use('clam') # Required to override system defaults
        ...
        if not self.app.history_restored_flag:
            self.btn_restore = tk.Button(btn_frame, text="Restore Previous Session History", command=self.do_restore_history, bg=XP_BTN_BG, relief="raised", bd=2, font=XP_FONT)
            self.btn_restore.pack(side=tk.LEFT, padx=(0, 10))

        tk.Button(btn_frame, text="Jump to Version", command=self.do_jump, bg=XP_BTN_BG, relief="raised", bd=2, font=XP_FONT_BOLD).pack(side=tk.LEFT)
        tk.Button(btn_frame, text="Close", command=self.destroy, bg=XP_BTN_BG, relief="raised", bd=2, font=XP_FONT).pack(side=tk.RIGHT)
