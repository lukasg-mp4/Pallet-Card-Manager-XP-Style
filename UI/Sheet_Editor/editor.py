...
class SheetEditor(tk.Frame):
    ...
    def setup_ui(self):
        # Print Button
        self.btn_print = tk.Button(self, text=f"Print Menu For: '{self.sheet_name}'", 
                                  command=self.open_print_manager, bg="#ffdddd", relief="raised", bd=2, font=XP_FONT)
        ...
        self.floating_entry = tk.Entry(self.canvas, textvariable=self.entry_var, 
                                      font=XP_FONT_CELL, justify="center", 
        ...
        tk.Button(btm, text="+ Add Rows", command=self.app_manager.row_manager.prompt_add_rows, bg=XP_BTN_BG, font=XP_FONT, relief="raised", bd=2).pack(side=tk.LEFT, padx=(10, 5), pady=5)
        tk.Button(btm, text="- Delete Rows", command=self.app_manager.row_manager.prompt_delete_rows, bg=XP_BTN_BG, font=XP_FONT, relief="raised", bd=2).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(btm, text="x Clear Sheet", command=self.app_manager.row_manager.prompt_clear_sheet, bg=XP_BTN_BG, font=XP_FONT, relief="raised", bd=2).pack(side=tk.RIGHT, padx=10, pady=5)
    ...
    def bind_events(self):
        ...
        self.floating_entry.bind("<Control-Return>", lambda e: self.move_cursor(-1, 0)) # Up
        self.floating_entry.bind("<Control-Tab>", lambda e: self.move_cursor(0, -1))    # Left
    ...
    def open_print_manager(self):
        if PrintDialog:
            self.hide_entry()
            try: 
                # We pass the class, not a module
