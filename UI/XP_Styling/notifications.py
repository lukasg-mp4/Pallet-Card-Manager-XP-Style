...
class XPInfoDialog(tk.Toplevel):
    def __init__(self, parent, title, message):
        ...
        tk.Label(content, text=message, bg=XP_BEIGE, font=("Tahoma", 10), wraplength=300, justify="center").pack(pady=(5, 15), expand=True)
        btn_frame = tk.Frame(content, bg=XP_BEIGE); btn_frame.pack(side=tk.BOTTOM, fill=tk.X)
        tk.Button(btn_frame, text="OK", width=10, command=self.destroy, bg=XP_BTN_BG, relief="raised", bd=2, font=XP_FONT).pack(side=tk.BOTTOM)
...
class XPErrorDialog(tk.Toplevel):
    def __init__(self, parent, title, message):
        ...
        tk.Label(content, text=message, bg=XP_BEIGE, font=("Tahoma", 10), wraplength=240, justify="center").pack(side=tk.LEFT, padx=(15, 0), pady=5, expand=True)
        btn_frame = tk.Frame(content, bg=XP_BEIGE); btn_frame.pack(side=tk.BOTTOM, fill=tk.X)
        tk.Button(btn_frame, text="OK", width=10, command=self.destroy, bg=XP_BTN_BG, relief="raised", bd=2, font=XP_FONT).pack(side=tk.BOTTOM)
