...
class XPDeleteSheetDialog(tk.Toplevel):
    def __init__(self, parent, sheet_list, current_selection):
        ...
        self.title_bar = XPTitleBar(main_frame, self, app_instance=self, title_text="Delete Sheet", close_func=self.cancel)
        ...
        self.combo = ttk.Combobox(content, textvariable=self.combo_var, values=sheet_list, state="readonly", font=("Tahoma", 9))
        ...
        tk.Button(btn_frame, text="OK", width=10, command=self.ok, bg=XP_BTN_BG, relief="raised", bd=2, font=XP_FONT).pack(side=tk.RIGHT, padx=5)
        tk.Button(btn_frame, text="Cancel", width=10, command=self.cancel, bg=XP_BTN_BG, relief="raised", bd=2, font=XP_FONT).pack(side=tk.RIGHT, padx=5)
